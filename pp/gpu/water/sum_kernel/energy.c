 //OpenCL calculation of energy of water box
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include "CL/cl.h"
...
int main ()
{
  cl_double cl_L, *summation, *atomsArray;
  cl_uint cl_nmol;
  int nthreads;
  printf("Enter number of threads: "); scanf("%i",&nthreads);
  printf("Number of GPU threads set to: %i\n",nthreads);
  cl_uint cl_nthreads = nthreads;
  cl_ulong time_start, time_end;
  long total_time;
  // alocation of variable to return partial sums of energy
  summation = malloc(sizeof(cl_double)*nthreads);
  ...
  //================ reading of configuration file - water box =====================
  int i,j,natoms,nmol;
  FILE *fg;
  char line[LENGTH], nothing[LENGTH];
  
  fg=fopen("../100k.gro", "r");
  fgets(line, LENGTH,fg); //skip first line
  fgets(line, LENGTH,fg); sscanf(line,"%i",&natoms);
  nmol=natoms/3; printf("Number of molecules %i\n",nmol);
  atomsArray = malloc(sizeof(cl_double)*nmol*9);  
  
  //reading of water molecules to a 1-DIMENSIONAL array
  for (i=0;i<nmol;i++){
    for(j=0;j<=2;j++){
      fgets(line, LENGTH,fg);
      sscanf(line, "%s %s %s %lf %lf %lf",nothing,nothing,nothing,
	  &atomsArray[i*9 + j*3 + 0], &atomsArray[i*9 + j*3 +1], &atomsArray[i*9 + j*3 + 2]);
    } 
  }
  printf("first line %lf  %lf  %lf\n",atomsArray[0],atomsArray[1],atomsArray[2]);
  fscanf(fg, "%lf",&L); // read box size
  printf("Box size %lf\n",L);
  fclose(fg);
  cl_nmol = nmol;  cl_L = L; //setting the variables to be sent to GPU
  
  //initiation of array summation, which will return results from kernel
...
// compiles (and links) kernel program AT RUNTIME!
  ...
  cl_mem input_buffer = clCreateBuffer(context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR, sizeof(cl_double)*nmol*9, atomsArray, NULL);
  //cl_mem output_buffer_summation = clCreateBuffer(context,NULL, sizeof(cl_double)*nthreads, NULL, NULL);
  cl_mem output_buffer_summation = clCreateBuffer(context,CL_MEM_WRITE_ONLY, sizeof(cl_double)*nthreads, NULL, NULL);
  cl_mem output_buffer_totalene = clCreateBuffer(context,CL_MEM_WRITE_ONLY, sizeof(cl_double), NULL, NULL);
  ...
  ret = clSetKernelArg(kernel, 1, sizeof(cl_uint),&cl_nmol);
  ret = clSetKernelArg(kernel, 2, sizeof(cl_mem),&input_buffer);
  ret = clSetKernelArg(kernel, 3, sizeof(cl_mem),&output_buffer_summation);
  ret = clSetKernelArg(kernel, 4, sizeof(cl_double),&cl_L);
  ret = clSetKernelArg(kernel, 5, sizeof(cl_double),&output_buffer_totalene);
//  cl_double totalene;
//  ret = clSetKernelArg(kernel, 5, sizeof(cl_double),&totalene);
  
  // setting global work size - # of cl_nthreads and dimension
  size_t global[1]; // setting the dimension
  global[0]= nthreads; // setting cl_nthreads
...
// synchronisation point {barrier}, waits for completion of list of events given by 2nd argument
  ret = clWaitForEvents(1, &prof_event);
    ...
    ret = clEnqueueReadBuffer(command_queue, output_buffer_summation, CL_TRUE, 0,
     	sizeof(cl_double)*nthreads, summation, 0, NULL, NULL);
    cl_double *total_energy;
    total_energy = malloc(sizeof(cl_double));
	ret = clEnqueueReadBuffer(command_queue, output_buffer_totalene, CL_TRUE, 0,
     	sizeof(cl_double), total_energy, 0, NULL, NULL);
	double hostenergy=0;
	for(i=0;i<nthreads;i++) {
	  hostenergy +=summation[i]; //sum partial sums
//      printf("summation[%i]=%lf\n",i,summation[i]);
	}
    double timeTaken=(double)total_time*1e-9;
   printf("Time taken = %.3lf seconds\n", timeTaken);
   printf("Box length %lf %lf\n",cl_L,L);
  printf("total energy calculated by kernel %lf\n",*total_energy);
  printf("Host energy  %lf per molecule %lf\n", hostenergy, hostenergy/nmol);  
  // releasing objects and memory
  free(summation);
  clReleaseMemObject(output_buffer_summation);
