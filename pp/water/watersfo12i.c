...
int main(){
    ...
    double npair_shared[omp_get_max_threads()];
	FILE *fp;
	...
	//#pragma omp parallel for private(i) reduction(+:energy) schedule(static, 1)
	//#pragma omp parallel for private(i) reduction(+:energy) schedule(static, 100)
	//#pragma omp parallel for private(i) reduction(+:energy) schedule(static, 10000)
	//#pragma omp parallel for private(i) reduction(+:energy) schedule(dynamic)
	//#pragma omp parallel for private(i) reduction(+:energy) schedule(dynamic, 1)
	//#pragma omp parallel for private(i) reduction(+:energy) schedule(dynamic, 100)
	//#pragma omp parallel for private(i) reduction(+:energy) schedule(dynamic, 10000)
	//#pragma omp parallel for private(i) reduction(+:energy) schedule(guided)
	//#pragma omp parallel for private(i) reduction(+:energy) schedule(guided, 1)
	//#pragma omp parallel for private(i) reduction(+:energy) schedule(guided, 100)
	//#pragma omp parallel for private(i) reduction(+:energy) schedule(guided, 10000)
	//#pragma omp parallel for private(i) reduction(+:energy) schedule(auto)
		for(i=0;i<nmol-1;i++){ // calculate energy as sum over all pairs
			int me = omp_get_thread_num();
            long npair = 0; // Private variable to count pairs of molecule per thread
			for(j=i+1;j<nmol;j++){
				energy=energy+energy12(i,j);
				npair++; // Increment npair for each evaluated pair
			}
            npair_shared[me] = npair;
			printf("Thread %d processed npair %ld\n", omp_get_thread_num(), npair); // Print npair by each thread
		}
		// Find min and max npair values among threads
		long minnpair = npair_shared[0];
		long maxnpair = npair_shared[0];
		...
		double load_imbalance = (double)(maxnpair - minnpair) / (double)(minnpair + maxnpair) / 2.0;
