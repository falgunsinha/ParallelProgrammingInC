...
void read_file(char conf[], int *nmol, double *box_size, double data[][3][3])
{
    FILE *fp;
    int natoms;
...
int main(int argc, char *argv[])
{
    ...
    int me, nproc, nmol, npair, npairmin, npairmax;
    ...
    MPI_Bcast(&conf, 20, MPI_CHAR, 0, MPI_COMM_WORLD);
    ...
    // Read
    read_file(conf, &nmol, &box_size, data);
