...
int main(){
	...
	//#pragma omp parallel for private(i) reduction(+:energy) schedule(auto)
		...
		// Load Imbalance Calculation within the parallel region
		#pragma omp single
		{
			int minnpair = npair_shared[0];
			int maxnpair = npair_shared[0];
			...
			double load_imbalance = 2.*(maxnpair-minnpair)/(0.+maxnpair+minnpair);
