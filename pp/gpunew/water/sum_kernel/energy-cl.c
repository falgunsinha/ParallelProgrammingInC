...
__kernel void energy(uint nthreads, uint nmol,
       __global double *atomArray, __global double *result,
       double L, __global double *total_energy )
{
int i;
...
// obtaining the index of a thread - analogue to MPI_Rank
...
// copy 9 coordinates to private memory
  for(u=0;u<9; u++)
    mol1[u]= atomArray[i*9+u]; */

  for(j=i+1;j<nmol;j++) //inner loop
  {
    for(xyz=0;xyz<3;xyz++)
    {
      dr[xyz] = atomArray[i*9+xyz] - atomArray[j*9+xyz];
    ...
    if(distsq<rcutsq){
      ...
      for(atomi=0;atomi<3;atomi++)
      {
        ...
        {
          for(xyz=0;xyz<3;xyz++)
            dr[xyz]= atomArray[i*9+atomi*3 + xyz] - atomArray[j*9+atomj*3 + xyz] + shift[xyz];
//          dist = pow(pow(dr[0],2)+pow(dr[1],2)+pow(dr[2],2),0.5);
//          dist = pow(dr[0]*dr[0]+dr[1]*dr[1]+dr[2]*dr[2],0.5);
//          dist = exp(log(dr[0]*dr[0]+dr[1]*dr[1]+dr[2]*dr[2])/2.);
...
// reduction of all partial sums to result[0] is carried by thread with ME==0
 if(me == 0) {
  *total_energy = 0.;
