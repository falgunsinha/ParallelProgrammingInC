...
// synchronisation point {barrier}, waits for completion of list of events given by 2nd argument
  ret = clWaitForEvents(1, &prof_event);
	...
	for(i=0;i<nthreads;i++) { //summing the final energy of threads serially
	  totalenergy+=summation[i]; //sum partial sums
//      printf("summation[%i]=%lf\n",i,summation[i]);
	}
	double timeTaken =(double)total_time*1e-9;
   printf("Time taken by GPU = %.3lf seconds\n", timeTaken);

   printf("Box length %lf %lf\n",cl_L,L);
  // print total energy calculated by kernel
  printf("Energy equals %lf per molecule %lf\n", totalenergy, totalenergy/nmol);  
  gettimeofday(&end, NULL); // returns structure with time in s and us (microseconds)
  dtime = ((end.tv_sec  - start.tv_sec)+(end.tv_usec - start.tv_usec)/1e6);
  printf("Total execution wall time: %f\n", dtime);
